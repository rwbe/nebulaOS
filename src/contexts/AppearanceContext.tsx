...
export const AppearanceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [wallpaper, setWallpaperState] = useState<Wallpaper>(() => {
    const saved = localStorage.getItem('win12-wallpaper');
    return saved ? JSON.parse(saved) : defaultWallpapers[1]; // default-dark
  });

  const [accentColor, setAccentColorState] = useState<string>(() => {
    const saved = localStorage.getItem('win12-accent-color');
    return saved || '217 100% 50%'; // Default NebulaOS blue
  });

  useEffect(() => {
    localStorage.setItem('win12-wallpaper', JSON.stringify(wallpaper));
  }, [wallpaper]);

  useEffect(() => {
    localStorage.setItem('win12-accent-color', accentColor);
